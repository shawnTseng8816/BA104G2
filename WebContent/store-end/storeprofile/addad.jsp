...
<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/store-end/storeprofile/sto.do" name="form1" enctype="multipart/form-data">
	<div class="container">
		...
		<div class="row">
			<div class="col-xs-12 col-sm-6">
			    ...
			    <input type="hidden" name="sa_no" size="45"
						 value="<%= (shopadVO==null)? "" : shopadVO.getSa_no()%>" />
						<!-- 記得hidden -->
		...
		<!-- 	       <input type="TEXT" name="sa_text" size="45" -->
		<%-- 				 value="<%= (shopadVO==null)? "保證能量充滿~戰鬥力十足" : shopadVO.getSa_text()%>" /> --%>
			       <span class="spin"></span>
			    </div>
			       ...
			       <label for="adblocksvc">廣告區塊編號</label>
			           ...
			           <select size="1" name="ab_no" size="45" id="selop" class="select op">
			         	 		...
			         	 		<c:forEach var="adblockVO" items="${adblockSvc.all}" > 
			          	 			<option value="${adblockVO.ab_no}">${adblockVO.ab_name}</option>
			         	 		</c:forEach>   
					   </select>
			       <span class="spin"></span>
			    </div>	
...
<%-- 							<img id="output1" name="upfile" height="330" width="550"  src="/BA104G2/GetPic?sa_no=${shopadVO.getSa_no()}&getType=shopad" > --%>
<%-- 						</c:if> --%>
						
<%-- 						<c:if test = "${shopadVO.getSa_no()==null}"> --%>
						<img id="output"  name="upfile">
<%-- 						</c:if>	 --%>
						<input type="file" name="upfile" id="output2" size="45" onchange="previewFile()"> 
						<!-- sa_img -->
			       <span class="spin"></span>
				...
				<div class="input">
			       <input type="hidden" name="action" value="insertad">
			       <input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>">
<!-- 				   <input type="hidden" name="action" value="upup"> -->
<%-- 						<c:if test = "${shopadVO!=null }"> --%>
<!-- 							<input type="hidden" name="upup" value="upup"> -->
<%-- 						</c:if> --%>
<%-- 						<c:if test="${shopadVO==null }"> --%>
<!-- 							<input type="hidden" name="upup" value="insertad"> -->
<%-- 						</c:if> --%>
...
<script>
  ...
  　       });
	    ...
	    function inPutInfo1(){
			...
			document.getElementsByName("sa_addtime")[0].value= '2017-12-07';
